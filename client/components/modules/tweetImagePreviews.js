...
const imagePreviews = (arrOfImages) => {
  if (arrOfImages.length === 1) {
    return (
      <img src={`/img/tweets/${arrOfImages[0]}`} className="tweet-photo" />
    );
  } else if (arrOfImages.length === 2) {
    return (
      <>
        <img
          src={`/img/tweets/${arrOfImages[0]}`}
          className="tweet-photo-2-first"
        />
        <img
          src={`/img/tweets/${arrOfImages[1]}`}
